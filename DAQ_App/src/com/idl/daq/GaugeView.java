...
public class GaugeView extends View {
    ...
    public static final float[] RANGE_VALUES = {0.0f, 25.0f, 40.0f, 100.0f};
    public static final int[] RANGE_COLORS = {Color.rgb(232, 111, 33),Color.rgb(27, 232, 33) , Color.rgb(232, 231, 33),
            Color.rgb(231, 32, 43)};
    
    public static final int TEXT_SHADOW_COLOR = Color.argb(100, 0, 0, 0);
    public static int TEXT_VALUE_COLOR = Color.WHITE;
    ...
    private void drawScale(final Canvas canvas) {
        ...
        for (int i = 0; i < totalTicks; i++) {
            ...
            if ((Math.abs(mod - 0) < 0.001) || (Math.abs(mod - div) < 0.001)) {
                ...
                canvas.drawText(valueString(((value-RANGE_VALUES[1])/((RANGE_VALUES[2]-RANGE_VALUES[1])/(max-min)))+min), 0.5f, y3 + 0.045f, paint);
            } else {
    ...
    public void setMinMaxWithPosition(float m1,float m2,float start,float end){
    	min=m1;
    	max=m2;
    	RANGE_VALUES[1]=start;
    	RANGE_VALUES[2]=end;
